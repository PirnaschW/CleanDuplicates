...
void  CScopeView::FillScopeView()
{
    ...
    const  std::filesystem::directory_entry  target{  LR"(D:\Projects\Test1)"  };
//const  std::filesystem::directory_entry  target{  LR"(E:\)"  };

    //  set  up  root  items
    hRoot  =  m_wndScopeView.InsertItem(_T("Scope"),  0,  0);
    m_wndScopeView.SetItemState(hRoot,  TVIS_BOLD,  TVIS_BOLD);
    hScope  =  m_wndScopeView.InsertItem(target.path().c_str(),  0,  0,  hRoot);
    m_wndScopeView.InsertItem(target.path().c_str(),  2,  2,  hScope);

    CollectFiles(target,  hRoot);
    m_wndScopeView.Expand(hRoot,  TVE_EXPAND);
}

void  CScopeView::CollectFiles(const  std::filesystem::directory_entry&  s,  HTREEITEM  hScope)
{
    for  (auto&  p  :  std::filesystem::directory_iterator(s,  std::filesystem::directory_options::skip_permission_denied))
    {
        ...
        {
            FileData  f{  p.path().c_str(),  false,  p.last_write_time(ec)  };
            ...
            if  (p.is_directory())
            {
                HTREEITEM  h  =  m_wndScopeView.InsertItem(p.path().filename().wstring().c_str(),  0,  0,  hScope);
                CollectFiles(p,  h);
                m_wndScopeView.Expand(h,  TVE_EXPAND);
            }
            else  m_wndScopeView.InsertItem(p.path().filename().wstring().c_str(),  2,  2,  hScope);
        }
    }
}
